/*
 * For the new MELOS node designed by Naveen
 *
 * */
...
int main (void)
{
   ...
   DDRD  &= ~ ((1<<4)|(1<<5)|(1<<6)|(1<<7));   // PD4->MSB and PD7->LSB

   DDRB  |= (1<<0);   // BT module and zigbee control
   PORTB |= (1<<0);   // High - Turn off BT module and zigbee control (PNP transister)

   DDRB  |= (1<<1);   // SD card module control
   PORTB &= ~(1<<1);  // Low - Turn on SD card (PNP transister)
...
if ((!(PIND & (1<<4))) && (!(PIND & (1<<5))) && (!(PIND & (1<<6))) && ((PIND & (1<<7))) )
return 1;
